...
// GPU family constants for kernel classification (mirrors MTLGPUFamily enum values)
// Used as pipeline.min_family to declare minimum device requirement
#define GGML_METAL_FAMILY_COMMON1  3001
#define GGML_METAL_FAMILY_COMMON2  3002
#define GGML_METAL_FAMILY_COMMON3  3003
#define GGML_METAL_FAMILY_APPLE1   1001
#define GGML_METAL_FAMILY_APPLE7   1007
#define GGML_METAL_FAMILY_METAL3   5001
#define GGML_METAL_FAMILY_METAL4   5002

// Vendor verification bitmask — which vendors has this kernel been tested on
// Default is 0 (unverified). Kernels must opt-in per vendor.
#define GGML_METAL_VERIFIED_APPLE  (1 << 0)
#define GGML_METAL_VERIFIED_AMD    (1 << 1)
#define GGML_METAL_VERIFIED_INTEL  (1 << 2)
...
// Helper: convert ggml_gpu_vendor enum to verified_vendors bitmask
static inline uint8_t ggml_metal_vendor_to_verified_bit(enum ggml_gpu_vendor vendor) {
    switch (vendor) {
        case GGML_GPU_VENDOR_APPLE:  return GGML_METAL_VERIFIED_APPLE;
        case GGML_GPU_VENDOR_AMD:    return GGML_METAL_VERIFIED_AMD;
        case GGML_GPU_VENDOR_INTEL:  return GGML_METAL_VERIFIED_INTEL;
        default:                     return 0;
    }
}
...
struct ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_ext        (ggml_metal_library_t lib, enum ggml_type tsrc0, enum ggml_type tsrc1, int nsg, int nxpsg, int r1ptg, bool use_shmem_reduce, bool src1_trans);
struct ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mm            (ggml_metal_library_t lib, const struct ggml_tensor * op);
struct ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv            (ggml_metal_library_t lib, const struct ggml_tensor * op, bool use_shmem_reduce, bool src1_trans);
...
struct ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_id         (ggml_metal_library_t lib, const struct ggml_tensor * op, bool use_shmem_reduce);
...
struct ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_irfft             (ggml_metal_library_t lib, const struct ggml_tensor * op);
struct ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_fold              (ggml_metal_library_t lib, const struct ggml_tensor * op);
...
struct ggml_metal_device_props {
    ...
    // GPU family classification (MTLGPUFamily enum values, 0 = not supported)
    // Used for kernel referential integrity — pipeline.min_family <= device.max_*_family
    int max_apple_family;   // highest MTLGPUFamilyAppleN supported
    int max_common_family;  // highest MTLGPUFamilyCommonN supported
    int max_metal_family;   // highest MTLGPUFamilyMetalN supported
...
// bandwidth tracking — cumulative counters for CPU↔GPU transfers (diagnostics only, not thread-safe)
struct ggml_metal_transfer_stats {
    int64_t h2d_bytes;    // host → device
    int64_t h2d_time_us;
    int64_t h2d_count;
    int64_t d2h_bytes;    // device → host
    int64_t d2h_time_us;
    int64_t d2h_count;
};

extern struct ggml_metal_transfer_stats ggml_metal_xfer_stats;
