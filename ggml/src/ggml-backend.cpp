...
float ggml_backend_dev_bench_matmul(ggml_backend_dev_t dev) {
    GGML_ASSERT(dev);

    // f16 4096x128 matmul — small enough for any GPU, large enough to measure
    const int M = 4096;
    const int K = 128;
    const int N = 1;
    ...
    // context: 2 input tensors + 1 output + graph overhead
    ...
    // zero-init tensors (values don't affect throughput measurement)
    ggml_backend_tensor_memset(a, 0, 0, ggml_nbytes(a));
    ggml_backend_tensor_memset(b, 0, 0, ggml_nbytes(b));
    ...
    // benchmark: loop until 500ms elapsed
    const int64_t target_us = 500 * 1000;
    int64_t total_us = 0;
    int runs = 0;
    ...
    float score = (total_us > 0) ? (float)runs / (float)total_us : 0.0f;
                ...
                if (ggml_backend_supports_op(sched->backends[b], node)) {
                    ...
                    if (n_supported > n_supported_best) {
                        ...
                        *node_backend_id = b;
                        ...
                        // warn if falling back to CPU after GPU rejection
                        if (b == sched->n_backends - 1) {  // CPU is last backend
                            // check if any higher-priority GPU backend rejected this op
                            bool gpu_rejected = false;
                            for (int g = 0; g < b; g++) {
                                if (!ggml_backend_supports_op(sched->backends[g], node)) {
                                    gpu_rejected = true;
                                    break;
                                }
                            }
                            ...
                            if (gpu_rejected) {
                                // deduplicate: warn once per (op_type, first_gpu_backend_name) pair
                                static std::set<std::pair<enum ggml_op, std::string>> warned;
                                std::string first_gpu_name = ggml_backend_name(sched->backends[0]);
                                auto key = std::make_pair(node->op, first_gpu_name);

                                if (warned.find(key) == warned.end()) {
                                    warned.insert(key);
                                    GGML_LOG_WARN("%s: op %s not supported on %s, falling back to CPU (may cause slowdown)\n",
                                        __func__, ggml_op_name(node->op), first_gpu_name.c_str());
                                }
                            }
                        }
                    }
