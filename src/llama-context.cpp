...
llama_context::llama_context(
    ...
    cparams.kv_device        = params.kv_device ? params.kv_device : "";
    ...
    if (!hparams.vocab_only) {
        ...
        // KV cache router: discover backend by name prefix
        if (!cparams.kv_device.empty()) {
            for (auto & backend : backends) {
                if (strncmp(ggml_backend_name(backend.get()), cparams.kv_device.c_str(), cparams.kv_device.size()) == 0) {
                    backend_kv = backend.get();
                    LLAMA_LOG_INFO("%s: kv-router: using backend %s for KV cache\n", __func__, ggml_backend_name(backend_kv));
                    break;
                }
            }
            if (!backend_kv) {
                // Backend not in model device list — scan all registered ggml devices
                // This handles the case where the KV device (e.g. Intel iGPU) is not used
                // for model layers but should be available for KV cache routing
                for (size_t i = 0; i < ggml_backend_dev_count(); ++i) {
                    ggml_backend_dev_t dev = ggml_backend_dev_get(i);
                    const char * dev_name = ggml_backend_dev_name(dev);
                    if (strncmp(dev_name, cparams.kv_device.c_str(), cparams.kv_device.size()) == 0) {
                        ggml_backend_t backend = ggml_backend_dev_init(dev, nullptr);
                        if (backend) {
                            // Insert before CPU (last element) — scheduler asserts CPU is last
                            auto it = backends.end() - 1;
                            backends.emplace(it, backend);
                            backend_kv = backend;
                            LLAMA_LOG_INFO("%s: kv-router: initialized and using backend %s for KV cache\n",
                                __func__, ggml_backend_name(backend_kv));
                        }
                        break;
                    }
                ...
                if (!backend_kv) {
                    LLAMA_LOG_WARN("%s: kv-router: backend '%s' not found, KV routing disabled\n", __func__, cparams.kv_device.c_str());
                }
            }
        }
    ...
    if (!hparams.vocab_only) {
        // Get buffer type for KV cache device (if specified)
        ggml_backend_buffer_type_t kv_buft = nullptr;
        if (backend_kv) {
            ggml_backend_dev_t kv_dev = ggml_backend_get_device(backend_kv);
            if (kv_dev) {
                kv_buft = ggml_backend_dev_buffer_type(kv_dev);
            }
        }
