...
// Thin dispatcher: shared preprocessing then dispatch to fused or unfused decode path.
// The fused path uses the Metal kernel (n_seqs == 1).
// The unfused path uses elementwise ops (multi-seq fallback).
...
ggml_tensor * llm_build_qwen35::build_layer_attn(
    ...
    // Qwen3.5 uses a single Q projection that outputs query + gate
...
ggml_tensor * llm_build_qwen35::build_layer_attn_linear(
    ...
    cb(beta, "beta", il);
    // build_lora_mm (mul_mat) output is contiguous — reshape is sufficient.
    ...
    // eliminating a separate ggml_cpy dispatch. For prompt/multi-seq, use legacy path.
    ggml_tensor * conv_output_proper;
    bool conv_state_written_by_kernel = false;

    if (n_seq_tokens == 1 && n_seqs == 1) {
        // Direct conv state write: kernel writes last (d_conv-1) elements to cache
        ...
        conv_state_written_by_kernel = true;
    } else {
        conv_output_proper = ggml_ssm_conv(ctx0, conv_input, conv_kernel);

        // Legacy path: extract and copy conv state back to cache
    ...
    // For decode (n_seq_tokens==1), the view_2d has only 1 row so it's contiguous
