...
ggml_tensor * llm_graph_context::build_attn_mha(
    ...
    // KV cache router: when KV buffers are on a separate device (e.g., Intel iGPU),
    // the scheduler auto-places KV ops there based on data locality.
    // Router op-pinning disabled — it creates excessive graph splits.
    // TODO: re-enable for fine-grained control when graph split count is optimized
...
ggml_tensor * llm_graph_context::build_rs(
    ...
    if (rs_zero >= 0) {
        ggml_tensor * state_zero = ggml_view_1d(ctx0, states, state_size, rs_zero*states->nb[1]);
    ...
    // Skip when there are no extra states (n_rs == n_seqs), which is the common case
    // for single-sequence decode. This avoids dispatching empty GPU kernels.
