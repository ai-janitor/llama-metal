...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    if (all_zero) {
        if (n_devices() > 1) {
            // auto-weighted split: micro-benchmark each device for compute throughput
            LLAMA_LOG_INFO("%s: benchmarking %zu devices for auto-split...\n", __func__, n_devices());
            for (size_t i = 0; i < n_devices(); ++i) {
                splits[i] = ggml_backend_dev_bench_matmul(devices[i]);
                LLAMA_LOG_INFO("%s:   %s: %.4f (score)\n", __func__,
